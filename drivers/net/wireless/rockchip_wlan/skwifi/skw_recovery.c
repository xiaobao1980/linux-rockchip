				...
				struct skw_iface *iface)
{
	...
	while (peer_map) {
		...
		if (!peer || test_bit(SKW_PEER_FLAG_DEAUTHED, &peer->flags))
			continue;
		...
		if (ether_addr_equal(peer->addr, core->bss.bssid)) {
			...
			set_bit(SKW_PEER_FLAG_DEAUTHED, &peer->flags);
...
skw_recovery_prepare(struct skw_core *skw, struct skw_recovery_data *rd)
{
	...
	for (i = 0; i < SKW_NR_IFACE; i++) {
		...
		rd->iface[i].peer_map = READ_ONCE(iface->peer_map);
...
void skw_recovery_del_peer(struct skw_iface *iface, u8 peer_idx)
{
	...
	if (rd->peer[peer_idx])
		set_bit(SKW_PEER_FLAG_DEAUTHED, &rd->peer[lmac_id][peer_idx]->flags);
