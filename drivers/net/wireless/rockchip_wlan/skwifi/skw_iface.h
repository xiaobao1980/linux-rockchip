...
#define SKW_PEER_FLAG_TAINT             (0)
#define SKW_PEER_FLAG_BAD_ID            (1)
#define SKW_PEER_FLAG_BAD_ADDR          (2)
#define SKW_PEER_FLAG_ACTIVE            (3)
#define SKW_PEER_FLAG_DEAUTHED          (4)

#define SKW_IFACE_FLAG_LEGACY_P2P_DEV   (0)
#define SKW_IFACE_FLAG_DEAUTH           (1)
#define SKW_IFACE_FLAG_OPENED           (2)
...
#define SKW_KEY_FLAG_WEP_SHARE        (0)
#define SKW_KEY_FLAG_WEP_UNICAST      (1)
#define SKW_KEY_FLAG_WEP_MULTICAST    (2)

struct skw_key_conf {
	u8 skw_cipher;
	u8 wep_idx;
	u16 resv;
	unsigned long installed_bitmap;
	unsigned long flags; /* reference to SKW_KEY_FLAG_ */
	struct mutex lock;
...
#define SKW_SM_FLAG_SAE_RX_CONFIRM     (0)

struct skw_sm {
	u8 *addr;
	u8 inst;
	u8 iface_iftype;
	unsigned long flags; /* reference SKW_SM_FLAG_ */
...
enum skw_msdu_filter {
	...
	SKW_MSDU_FILTER_ICMP,
	SKW_MSDU_FILTER_IGMP,
	SKW_MSDU_FILTER_VOWIFI = 10,
...
#define SKW_RX_FILTER_DBG       (BIT(SKW_MSDU_FILTER_EAP)  |  \
				 ...
				 BIT(SKW_MSDU_FILTER_ARP)  |  \
				 BIT(SKW_MSDU_FILTER_DHCP) |  \
				 BIT(SKW_MSDU_FILTER_ICMP) |  \
				 BIT(SKW_MSDU_FILTER_IGMP) |  \
				 BIT(SKW_MSDU_FILTER_DHCPV6))
...
struct skw_peer {
	...
	u16 channel;
	u16 rx_tid_map;
	unsigned long flags; /* reference SKW_PEER_FLAG_ */
	__be32 ip_addr;
...
struct skw_iface {
	...
	unsigned long peer_map;
	unsigned long flags;  /* reference SKW_IFACE_FLAG_ */
