...
static int skw_set_cca_enable_mib(struct wiphy *wiphy, struct net_device *dev,
			    bool enable)
{
	struct skw_iface *iface;
...
static void skw_count_iftype(struct wiphy *wiphy, int num[NUM_NL80211_IFTYPES])
{
	...
	for (i = 0; i < SKW_NR_IFACE; i++) {
		...
		if (!iface || iface->wdev.iftype == NL80211_IFTYPE_P2P_DEVICE ||
		    test_bit(SKW_IFACE_FLAG_LEGACY_P2P_DEV, &iface->flags))
			continue;
			   ...
			   struct vif_params *params)
{
	...
	skw_dbg("%s (inst: %d), %s -> %s, mac: %pM, 4addr: %d, flags: 0x%lx\n",
			...
			const u8 *addr, struct key_params *params)
{
	...
	set_bit(key_idx, &conf->installed_bitmap);
	...
	if (cipher == SKW_CIPHER_TYPE_WEP40 ||
		...
		set_bit(SKW_KEY_FLAG_WEP_SHARE, &conf->flags);
		return 0;
	}
	...
	if (ret) {
		...
		clear_bit(key_idx, &conf->installed_bitmap);
		kfree_rcu(key, rcu);
	}
			...
			int key_type, const u8 *addr)
{
	...
	if (test_bit(key_idx, &conf->installed_bitmap)) {
	...
	if (test_bit(SKW_KEY_FLAG_WEP_SHARE, &conf->flags)) {
		clear_bit(SKW_KEY_FLAG_WEP_SHARE, &conf->flags);
		clear_bit(SKW_KEY_FLAG_WEP_UNICAST, &conf->flags);
		clear_bit(SKW_KEY_FLAG_WEP_MULTICAST, &conf->flags);
	}
			       ...
			       u8 key_idx, bool unicast, bool multicast)
{
	...
	if (unicast) {
		...
		if (ret)
			set_bit(SKW_KEY_FLAG_WEP_UNICAST, &conf->flags);
	}
	...
	if (multicast) {
		ret = skw_cmd_add_key(wiphy, dev, conf->skw_cipher,
				      ...
				      key_data, key_len, mac);
		if (ret)
			set_bit(SKW_KEY_FLAG_WEP_MULTICAST, &conf->flags);
	}
			...
			struct cfg80211_ap_settings *settings)
{
	...
	if (test_bit(SKW_KEY_FLAG_WEP_UNICAST, &conf->flags) ||
	    test_bit(SKW_KEY_FLAG_WEP_MULTICAST, &conf->flags))
		__skw_set_default_key(wiphy, dev, 0, conf->wep_idx,
				test_bit(SKW_KEY_FLAG_WEP_UNICAST, &conf->flags),
				test_bit(SKW_KEY_FLAG_WEP_MULTICAST, &conf->flags));
	...
	clear_bit(SKW_IFACE_FLAG_DEAUTH, &iface->flags);
			...
			struct skw_peer_ctx *ctx, u8 subtype, u16 reason)
{
	...
	if (ctx->peer) {
		...
		tx = !test_and_set_bit(SKW_PEER_FLAG_DEAUTHED, &ctx->peer->flags);
	}
			...
			u8 subtype, u16 reason)
{
	...
	struct skw_peer_ctx *ctx;
	struct skw_core *skw = wiphy_priv(wiphy);
	unsigned long peer_map = READ_ONCE(iface->peer_map);
		...
		SKW_NULL)
{
	...
	set_bit(SKW_IFACE_FLAG_DEAUTH, &iface->flags);
	clear_bit(iface->id, &skw->hw.lmac[iface->lmac_id].iface_bitmap);
	...
	if (ret) {
		clear_bit(SKW_IFACE_FLAG_DEAUTH, &iface->flags);
		set_bit(iface->id, &skw->hw.lmac[iface->lmac_id].iface_bitmap);
		    ...
		    struct station_parameters *params)
{
	...
	if (test_bit(SKW_KEY_FLAG_WEP_SHARE, &iface->key_conf.flags))
...
static int skw_scan(struct wiphy *wiphy, struct cfg80211_scan_request *req)
{
	...
	for (i = 0; i < req->n_channels; i++) {
		...
		if (test_bit(SKW_FLAG_OUTDOOR, &skw->flags) &&
			req->channels[i]->flags & IEEE80211_CHAN_INDOOR_ONLY) {
			continue;
		}
		    ...
		    struct cfg80211_bss *bss, bool roaming)
{
	...
	skw_set_cca_enable_mib(wiphy, ndev, iface->skw->config.fw.cca_en);
		    ...
		    struct cfg80211_auth_request *req)
{
	...
	if (auth_data && le16_to_cpu(*((u16 *)auth_data) == 2) &&
	    test_bit(SKW_SM_FLAG_SAE_RX_CONFIRM, &iface->sta.core.sm.flags))
...
{
	if (test_bit(SKW_CONN_FLAG_ASSOCED, &conn->flags)) {
	...
	set_bit(SKW_CONN_FLAG_ASSOCED, &conn->flags);
}
	...
	int resp_ie_len, bool local_gen, gfp_t gfp)
{
	...
	clear_bit(SKW_CONN_FLAG_ASSOCED, &conn->flags);
		...
		struct skw_connect_param *conn)
{
	...
	req.use_mfp = !!test_bit(SKW_CONN_FLAG_USE_MFP, &conn->flags);
	req.flags = conn->flags & 0xffffffff;
			...
			struct cfg80211_connect_params *req)
{
	...
	if (req->auth_type == NL80211_AUTHTYPE_AUTOMATIC) {
		...
		set_bit(SKW_CONN_FLAG_AUTH_AUTO, &conn->flags);
	}
	...
	if (req->key && req->key_len) {
		...
		set_bit(SKW_CONN_FLAG_KEY_VALID, &conn->flags);
	}
...
static inline void __skw_set_peer_flags(struct skw_peer_ctx *ctx, u32 flag)
{
	if (ctx) {
		skw_peer_ctx_lock(ctx);

		if (ctx->peer)
			set_bit(flag, &ctx->peer->flags);
