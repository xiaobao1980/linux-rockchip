...
TRACE_EVENT(skw_edma_tx_isr,
	    TP_PROTO(u32 channel, u64 start, u64 end, int num),
	    TP_ARGS(channel, start, end, num),
	    ...
	    TP_printk("channel: %d, head: 0x%llx, tail: 0x%llx, num: %d",
		      __entry->channel, __entry->start,
		      __entry->end, __entry->num)
);

TRACE_EVENT(skw_edma_tx_complete_isr,
	    TP_PROTO(u32 channel, u64 start, u64 end, int num),
	    TP_ARGS(channel, start, end, num),
...
TRACE_EVENT(skw_edma_tx_poll,
	    TP_PROTO(u32 channel, int node_idx, u64 addr, int done, int xmit_len),
	    TP_ARGS(channel, node_idx, addr, done, xmit_len),

	    TP_STRUCT__entry(
	    __field(u32, channel)
	    __field(int, node_idx)
	    __field(u64, addr)
	    __field(int, done)
	    __field(int, xmit_len)
	    ),

	    TP_fast_assign(
	    __entry->channel = channel;
	    __entry->node_idx = node_idx;
	    __entry->addr = addr;
	    __entry->done = done;
	    __entry->xmit_len = xmit_len;
	    ),

	    TP_printk("channel: %d, node[%d]: 0x%llx, xmit_len: 0x%d, done: %d",
		      __entry->channel, __entry->node_idx,
		      __entry->addr, __entry->xmit_len, __entry->done)
);

TRACE_EVENT(skw_edma_rx_ring_isr,
	    TP_PROTO(u32 channel, u64 start, u64 end, int num),
	    TP_ARGS(channel, start, end, num),
...
TRACE_EVENT(skw_edma_rx_complete_isr,
	    TP_PROTO(u32 channel, u64 start, u64 end, int num),
	    TP_ARGS(channel, start, end, num),
...
TRACE_EVENT(skw_edma_set_rx_ring,
	    TP_PROTO(u32 channel, int qlen),
	    TP_ARGS(channel, qlen),
	    ...
	    TP_printk("channel: %d, qlen: %d",
		      __entry->channel, __entry->qlen)
);

TRACE_EVENT(skw_edma_rx_filter_isr,
	    TP_PROTO(u32 channel, u64 start, u64 end, int num),
	    TP_ARGS(channel, start, end, num),
